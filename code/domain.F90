...
! if (rank .eq. size0-1) print *, "xinject2=", xinject2
...
!				,10000)
		buffsize = max(10*int(ppc0*upsamp_e*c*max((mx-nghost)*(my-nghost),1*(mx-nghost)*(mz-nghostz))) &
				,10000)	
				
#else
!		buffsize = max(3*int(ppc0*c*(1*(mx-5))),60000)
		buffsize = max(3*int(ppc0*upsamp_e*c*max((mx-nghost),(my-nghost))),60000)
...
!	enddo
		...
		mxcum=sum(mxl(i1:i2)-nghost)-(mxl(rank+1)-nghost)
...
!				,10000)
		...
		buffsize = max(10*int(ppc0*c*max((mx-nghost)*(my-nghost),1*(mx-nghost)*(mz-nghostz))) &
				,10000)	
		
#else
!		buffsize = max(3*int(ppc0*c*(1*(mx-5))),60000)
		buffsize = max(3*int(ppc0*c*max((mx-nghost),(my-nghost))),60000)
