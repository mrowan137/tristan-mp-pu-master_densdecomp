...
!-------------------------------------------------------------------------------
	...
	logical :: background
 integer :: rstart_jump,rstep_jump,rstep_first
 integer :: lstart_jump,lstep_jump,lstep_first
 real(sprec) :: u_sh_in, u_sh_left
!-------------------------------------------------------------------------------
...
!	call inputpar_geti_def("particles", "split_e", 1, split_e) !move the wall to be splitdomain behind the splitter	
 ...
 rstart_jump = rightclean_start
 rstep_jump = rightclean_interval
...
if(rank .eq. 0) print *,"binit0=", binit

!	Binit=sqrt((gamma0 -1)*ppc0*.5*c**2*(mi+me)*sigma)
!if(rank .eq. 0) print *, "binit1=", binit
...
!	integer :: upsamp_e_local, upsamp_i_local
	...
	leftwall=20.
	xinject=leftwall
	xinject2=mx0-30. !min(50.,(mx0-2.)) !mx0-50.
        walloc=leftwall
	! ------------------------------------------
...
subroutine inject_particles_user_lor()
...
end subroutine inject_particles_user_lor
...
!    rightclean=0.
	...
	call inject_from_wall(x1,x2,y1,y2,z1,z2,ppc,gamma_drift,&
	  delgam_i,delgam_e,wall_speed,weight,upsamp_e,upsamp_i)
...
!    if(lap .gt. 600) ppc =0 !hack
    ...
    call inject_from_wall(x1,x2,y1,y2,z1,z2,ppc,gamma_drift,delgam_i, &
       	  delgam_e,injector_speed,weight, upsamp_e,upsamp_i)
...
!	call MPI_BARRIER(MPI_COMM_WORLD,ierr)
	...
	end subroutine field_bc_user_orig
...
rw=.true.
!rw=.false. !hack
	if(rightclean .eq. 1 .and. lap .gt. rightclean_start .and. modulo(lap, rightclean_interval) .eq. 1 ) rw=.false.
	if(wall .and. rightwall > 0 .and. rw) then 
...
!unwinds the position of the particle by the full timestep. 
	...
	end subroutine particle_bc_user !_orig


	subroutine particle_bc_user_lor()
	implicit none
	real invgam, gammawall, betawall, gamma, y0, z0, betawall2, gammawall2
	real ycolis,zcolis,q0,tfrac 
	real(sprec) :: x0, xcolis
        real(sprec) :: walloc,walloc0,wall2loc,wall2loc0
	integer n1,i0,i1, iter,wall2counter
	logical in,wall2bool
        real betainj, leftcleanoffset
...
!unwinds the position of the particle by the full timestep. 
	      ...
	      if(wall2bool .eq. .true. .and. p(n1)%x .gt. wall2loc+1e6*0.) then 
...
!unwinds the position of the particle by the full timestep. 
 ...
 if (leftclean .eq. 1 .and. lap .ge. lstart_jump) then
	...
	end subroutine particle_bc_user_lor
