...
subroutine bc_b1()
	...
	if(sizex .ne. 1) then
		...
		if(periodicx.eq.1) then
                        do iter=1,nghost/2
                           call copylayrx(bx,by,bz,mx,my,mz,&
                                nghost/2+1-iter,mx-(nghost/2+iter),mx-(nghost/2+1)+iter,nghost/2+iter)
                        enddo
			if( highorder) then
	...
	if(sizey .ne. 1) then
		...
		if(periodicy.eq.1) then
                        do iter=1,nghost/2
                           call copylayry(bx,by,bz,mx,my,mz,&
                                nghost/2+1-iter,my-(nghost/2+iter),my-(nghost/2+1)+iter,nghost/2+iter)
                        enddo     
			if( highorder) then
...
subroutine bc_e1()
	...
	if(periodicz.eq.1) then
		...
		!transfer z information among processors, not periodic between 0,n-1
                do iter=1,nghostz/2
                   call copy_layrz2_opt(ex,ey,ez,mx,my,mz,&
                        nghostz/2+1-iter,mz-(nghostz/2+iter),mz-(nghostz/2+1)+iter,nghostz/2+iter)
                enddo
		if( highorder) then
...
subroutine copy_layrz2(bx,by,bz,mx,my,mz,lt,ls,nt,ns)
	...
	call MPI_SendRecv(by(:,:,mz-(nghostz/2+1)),count,mpi_read,uprank,uptag, &
	bufferin,   count,mpi_read,dwnrank,uptag, &
	comm,status,ierr)
	
	if(rank/(sizex*sizey) .ne. 0) by(:,:,nghostz/2)=bufferin
...
!-------------------------------------------------------------------------------
! 						subroutine exchange_current (CLEANUP)					 
!																		
! 
!  							
!-------------------------------------------------------------------------------
...
subroutine exchange_current()
	...
	integer :: uprank, dwnrank, comm,count,count2,uptag,dwntag,ierr, &
	...
	!add periodic from x
	 ...
	 if(periodicx .eq. 1) then
		...
		bufferin1x(1:nghost/2+1,:,:)=curx((mx-nghost/2):(mx),:,:)
		bufferin2x(1:nghost/2,:,:)=curx(1:nghost/2,:,:)
		curx((nghost/2+1):(nghost),:,:)=curx((nghost/2+1):(nghost),:,:)+bufferin1x(1:nghost/2+1,:,:) 
		curx((mx-(nghost-1)):(mx-(nghost/2+1)),:,:)=curx((mx-(nghost-1)):(mx-(nghost/2+1)),:,:)+bufferin2x(1:nghost/2,:,:) 
		
		bufferin1x(1:nghost/2+1,:,:)=cury((mx-nghost/2):(mx),:,:)
		bufferin2x(1:nghost/2,:,:)=cury(1:nghost/2,:,:)
		cury((nghost/2+1):(nghost),:,:)=cury((nghost/2+1):(nghost),:,:)+bufferin1x(1:nghost/2+1,:,:) 
		cury(mx-(nghost-1):(mx-(nghost/2+1)),:,:)=cury(mx-(nghost-1):(mx-(nghost/2+1)),:,:)+bufferin2x(1:nghost/2,:,:) 
		
		bufferin1x(1:nghost/2+1,:,:)=curz((mx-nghost/2):(mx),:,:) 
		bufferin2x(1:nghost/2,:,:)=curz(1:nghost/2,:,:)
		curz((nghost/2+1):(nghost),:,:)=curz((nghost/2+1):(nghost),:,:)+bufferin1x(1:nghost/2+1,:,:)
		curz((mx-(nghost-1)):(mx-(nghost/2+1)),:,:)=curz((mx-(nghost-1)):(mx-(nghost/2+1)),:,:)+bufferin2x(1:nghost/2,:,:)
...
#ifdef twoD
		count=(j2-j1+1)*(nghost/2+1)
                count2=(j2-j1+1)*(nghost/2)
		k1=1
		k2=1
#else
		count=(j2-j1+1)*(k2-k1+1)*(nghost/2+1)
...
!	 		 
		...
		curx(:,(my-(nghost-1)):(my-(nghost/2+1)),:)=curx(:,(my-(nghost-1)):(my-(nghost/2+1)),:)+bufferin2y(:,1:nghost/2,:) 
		...
		cury(:,(my-(nghost-1)):(my-(nghost/2+1)),:)=cury(:,(my-(nghost-1)):(my-(nghost/2+1)),:)+bufferin2y(:,1:nghost/2,:) 
		...
		curz(:,(my-(nghost-1)):(my-(nghost/2+1)),:)=curz(:,(my-(nghost-1)):(my-(nghost/2+1)),:)+bufferin2y(:,1:nghost/2,:)
...
!		if(rank/sizex.eq.sizey-1 .and. periodicy .ne. 1) iperiodic=.false.
		...
		call MPI_SendRecv(curz(i1:i2,(my-(nghost/2)):(my),k1:k2),count,mpi_read,rgtrank &
		,rgttag,bufferin1y(i1:i2,1:nghost/2+1,k1:k2),count,mpi_read,lftrank,rgttag &
		,comm,status,ierr)
...
!		if(rank/sizex.eq.0 .and. periodicy .ne. 1) iperiodic=.false.
		...
		call MPI_SendRecv(curz(i1:i2,1:nghost/2,k1:k2),count2,mpi_read,lftrank,lfttag, &
		bufferin2y(i1:i2,1:nghost/2,k1:k2),count2,mpi_read,rgtrank,lfttag, &
		comm,status,ierr)
		...
		!filter in z direction
			...
			if(periodicz.eq.1) then
				!call copy_layrz1(curx,cury,curz,mx,my,mz,2,mz-3,mz-2,3)
                                ! I think bug in the PU version, for this line
