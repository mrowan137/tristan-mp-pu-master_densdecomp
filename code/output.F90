...
!-------------------------------------------------------------------------------
	...
	character (len=180) :: fnametrq
	character (len=180) :: fnameprt, fnamefld
			...
			if(xbin .ge. 1 .and. xbin .le. nbins) then
				...
				if(gbin .ge. 1 .and. gbin .le. gambins ) then 				
					...
					-real(p(i)%splitlev))*p(i)%ch                
                                        !print *,"added: ",real(splitratio)**(1.-real(p(i)%splitlev))*p(i)%ch   
				endif
       ...
       !(electron) flow mean velocity ***********
		...
		if(rank .eq. 0) then     
			...
			fname="output/spect."//trim(indchar)
                        fname=trim(output_dir_name)//"/spect."//trim(indchar)
            print *,"",fname
...
!       specelprime=specein
		...
		if(rank .eq. 0) then     
			...
			fname="output/spect."//trim(indchar)
                        fname=trim(output_dir_name)//"/spect."//trim(indchar)
            if(rank.eq.0) print *,"fname=",fname
  ...
  IF(lap.ge.pltstart.and.modulo((lap-pltstart),interval).eq.0) THEN
     ...
     do var=1,3             !loop over dimensions
        ...
        if(rank .eq. 0) then 
           ...
           fname=trim(output_dir_name)//"/momentum."//trim(indchar)
           if(var.eq.1) then 
              if(rank.eq.0)PRINT *, "fname=", fname
              open(unit=11,file=fname,form='unformatted')
              close(11,status='delete')
...
!  integer(HID_T) :: plist_id ! Property list identifier
  ...
  if(lap .ge.pltstart .and. modulo((lap-pltstart),interval) .eq.0) then
     ...
     fparam=trim(output_dir_name)//"/param."//trim(indchar)
     if(rank .eq. 0) then
        print *,"fparam=",fparam
...
subroutine output_tot()
  ...
  integer ::all_strd_ions(size0), all_strd_lecs(size0)
  integer*8 all_strd_ions_lng(size0), all_strd_lecs_lng(size0)
  ...
  integer*4,allocatable:: int_temp_vec(:)
  integer*4,allocatable:: int_temp0_vec(:)

  integer*4,allocatable:: ions_str_ind(:)
  integer*4,allocatable:: lecs_str_ind(:)
  integer*8, allocatable:: selecti_vec(:),selecte_vec(:) 
  integer :: status(MPI_STATUS_SIZE)
  integer ::tmp1,tmp1i,tmp1e
  integer :: strd_ions, strd_lecs 
  logical :: saverank
...
!     write(fnameprt,"(a16,i3.3)") "output/prtl.tot.",ind     
     ...
     fnamefld=trim(output_dir_name)//"/flds.tot."//trim(indchar)
     fnameprt=trim(output_dir_name)//"/prtl.tot."//trim(indchar)
     ...
     !FLDS.TOT. saving
     if (rank.eq.0)print *,"fnamefld=",fnamefld
...
!---------------------------------------------------------------
     ...
     tmp1 = ions
     do n=1,ions
        if (modulo(p(n)%ind/2, stride) .eq. 0) then
           strd_ions=strd_ions+1
        endif
     enddo
     
     tmp1 = max(tmp1,1)
     !I think setting strd_ions to 1 when it should be
     !zero is causing errors
     !strd_ions =max(strd_ions,1)
     

        
     call mpi_allgather(strd_ions,1,mpi_integer,all_strd_ions,1 &
          ,mpi_integer,mpi_comm_world, error)
     call mpi_allgather(tmp1,1,mpi_integer,all_ions,1 &
          ,mpi_integer,mpi_comm_world, error)
     tmp1 = 1
     !!! Make an array of indices:
     allocate(ions_str_ind(max(strd_ions,1)))
     if (strd_ions .ne. 0) then
     ...
     do n=maxhlf+1,maxhlf+lecs
        if (modulo(p(n)%ind/2, stride) .eq. 0) then
           strd_lecs =strd_lecs+ 1
        endif
     enddo

     tmp1 = max(tmp1,1)

     !strd_lecs=max(strd_lecs,1)
     
     call mpi_allgather(strd_lecs,1,mpi_integer,all_strd_lecs,1, &
          mpi_integer,mpi_comm_world, error)
     ...
     tmp1 = 1
     !!! Make an array of indices:
     allocate(lecs_str_ind(max(strd_lecs,1)))

     if (strd_lecs .ne. 0) then
        do n = maxhlf+1,maxhlf+lecs
           if (modulo(p(n)%ind/2, stride) .eq. 0) then
              lecs_str_ind(tmp1)=n
              tmp1 =tmp1+ 1
     ...
     all_strd_ions_lng = all_strd_ions
     all_strd_lecs_lng = all_strd_lecs
#else
     all_ions_lng=ions
     all_lecs_lng=lecs
#endif    
     ...
     dimsfions(1)=sum(all_strd_ions_lng)
     dimsflecs(1)=sum(all_strd_lecs_lng)
     ...
     if(rank .eq. 0 .and. debug) then 
        ...
        print *,"dimsions", sum(all_strd_ions_lng)
        print *,"dimslecs", sum(all_strd_lecs_lng)
     endif
        ! allocate arrays
     allocate(temporary_vec(max(max(strd_ions,strd_lecs),1)))
     allocate(int_temp_vec(max(max(strd_ions,strd_lecs),1)))       
#ifdef serIO
     if (rank .eq. 0) then
        allocate(temporary0_vec(max(maxval(all_strd_ions_lng), &
             maxval(all_strd_lecs_lng))))
        temporary0_vec(:)=0.

        allocate(int_temp0_vec(max(maxval(all_strd_ions_lng), &
             maxval(all_strd_lecs_lng))))
        int_temp0_vec(:)=0.
     endif
#endif  
     ...
     do i=1,nvars
        ...
        !IONS
        if (strd_ions .ne. 0) then
           if(varname.eq.'xi') temporary_vec(1:strd_ions)=p(ions_str_ind)%x &
        	 +mxcum
           if(varname.eq.'yi') temporary_vec(1:strd_ions)=p(ions_str_ind)%y &
             +mycum
           if(varname.eq.'zi') temporary_vec(1:strd_ions)=p(ions_str_ind)%z &
        	 +mzcum
           if(varname.eq.'ui') temporary_vec(1:strd_ions)=p(ions_str_ind)%u
           if(varname.eq.'vi') temporary_vec(1:strd_ions)=p(ions_str_ind)%v
           if(varname.eq.'wi') temporary_vec(1:strd_ions)=p(ions_str_ind)%w    
           if(varname.eq.'chi') temporary_vec(1:strd_ions)=p(ions_str_ind)%ch
           if(varname.eq.'gammai') temporary_vec(1:strd_ions)=sqrt(1. &
             +(p(ions_str_ind)%u**2+p(ions_str_ind)%v**2 &
             +p(ions_str_ind)%w**2))
        !!! NOW WE HAVE TO DO INTEGERS
           if(varname.eq.'indi') int_temp_vec(1:strd_ions)=p(ions_str_ind)%ind
           if(varname.eq.'proci') int_temp_vec(1:strd_ions)=p(ions_str_ind)%proc
        endif !end ions
        !ELECTRONS
        if (strd_lecs .ne. 0) then
           if(varname.eq.'xe') temporary_vec(1:strd_lecs)=p(lecs_str_ind)%x+mxcum
           if(varname.eq.'ye') temporary_vec(1:strd_lecs)=p(lecs_str_ind)%y+mycum
           if(varname.eq.'ze') temporary_vec(1:strd_lecs)=p(lecs_str_ind)%z+mzcum
           if(varname.eq.'ue') temporary_vec(1:strd_lecs)=p(lecs_str_ind)%u
           if(varname.eq.'ve') temporary_vec(1:strd_lecs)=p(lecs_str_ind)%v
           if(varname.eq.'we') temporary_vec(1:strd_lecs)=p(lecs_str_ind)%w
           if(varname.eq.'che') temporary_vec(1:strd_lecs)=p(lecs_str_ind)%ch
           if(varname.eq.'gammae') then
              temporary_vec(1:strd_lecs)=sqrt(1. &
                   +(p(lecs_str_ind)%u**2+p(lecs_str_ind)%v**2+p(lecs_str_ind)%w**2))
           endif
           if(varname.eq.'inde')int_temp_vec(1:strd_lecs)=p(lecs_str_ind)%ind
           if(varname.eq.'proce')int_temp_vec(1:strd_lecs)=p(lecs_str_ind)%proc
        endif !end electrons
#ifdef serIO
        !
        ...
        do procn=0,size0-1
           ...
           if (rank .eq. 0) then
              ...
              if (procn .ne. 0) then !receive from procn                                
#ifdef MPI
                 if(i .le. midvars .and. all_strd_ions_lng(procn +1) .ne.0) then
                    if (varname .eq. 'indi' .or. varname .eq. 'proci') then
                       call MPI_Recv( &
                            int_temp0_vec(1:all_strd_ions_lng(procn+1)) &
                            ,all_strd_ions_lng(procn+1), &
                            mpi_integer,procn,1, &
                            MPI_COMM_WORLD,status,error)
                       
                    else
                       call MPI_Recv( &
                            temporary0_vec(1:all_strd_ions_lng(procn+1)) &
                            ,all_strd_ions_lng(procn+1), &
                            mpi_real,procn,1, &
                            MPI_COMM_WORLD,status,error)
                    endif
                 else
                    if(all_strd_lecs_lng(procn +1) .ne. 0 ) then
                       if (varname .eq. 'inde' .or. &
                            varname .eq. 'proce') then
                          call MPI_Recv( &
                               int_temp0_vec(1:all_strd_lecs_lng(procn+1)) &
                               ,all_strd_lecs_lng(procn+1), &
                               mpi_integer,procn,1, &
                               MPI_COMM_WORLD,status,error)
                       else
                       
                          call MPI_Recv( &
                               temporary0_vec(1:all_strd_lecs_lng(procn+1)) &
                               ,all_strd_lecs_lng(procn+1) &
                               ,mpi_real,procn,1, &
                               MPI_COMM_WORLD,status,error)
                       endif
                 ...
                 if (i .le. midvars .and. all_strd_ions_lng(1) .ne. 0) then
                    if (varname .eq. 'indi' .or. varname .eq. 'proci') then
                       int_temp0_vec(1:all_strd_ions_lng(1))= &
                         int_temp_vec(1:all_strd_ions_lng(1))   

                    else
                       temporary0_vec(1:all_strd_ions_lng(1))= &
                        temporary_vec(1:all_strd_ions_lng(1))   
                    endif
                 else
                    if (all_strd_lecs_lng(1).ne. 0) then
                       if(varname .eq. 'inde' .or.&
                            varname .eq. 'proce') then
                          int_temp0_vec(1:all_strd_lecs_lng(1))= &
                               int_temp_vec(1:all_strd_lecs_lng(1))  
                       else
                          temporary0_vec(1:all_strd_lecs_lng(1))= &
                               temporary_vec(1:all_strd_lecs_lng(1))  
                       endif ! end if varname eq 'inde
                    endif !end if lecs_lng(1) != 0)
                 endif ! end if i .le. midvars
              endif
              
              !define count and offset
              if(i.le.midvars)countpart =all_strd_ions_lng(procn+1)
              if(i.gt.midvars)countpart =all_strd_lecs_lng(procn+1)
              
              offsetpart = 0
              if(procn .gt. 0) then 
                 if(i .le. midvars) then
                    offsetpart = sum(all_strd_ions_lng(1:procn))
                 else
                    offsetpart = sum(all_strd_lecs_lng(1:procn))
                 endif
              endif
              ...
              if (procn .eq. 0) then
                 if (varname .eq. 'inde' .or. varname .eq.'indi' .or. &
                      varname .eq. 'proci' .or. varname .eq. 'proce') then
                    call h5dcreate_f(file_id,varname,H5T_NATIVE_INTEGER, &
              ...
              if(i .le. midvars .and. all_strd_ions_lng(procn+1) .ne. 0) then
                 if (varname .eq. 'indi' .or. varname .eq. 'proci') then
                    call h5dwrite_integer_1(dset_id(i), H5T_NATIVE_INTEGER, &
                      int_temp0_vec(1:all_strd_ions_lng(procn+1)),  &
                      dimsfiions,error,memspace,filespace(i))
                 else
                 ...
                 if (all_strd_lecs_lng(procn+1) .ne. 0) then
                    if (varname .eq. 'inde' .or. varname .eq. 'proce') then
                       call h5dwrite_integer_1(dset_id(i), H5T_NATIVE_INTEGER, &
                            int_temp0_vec(1:all_strd_lecs_lng(procn+1)),  &
                            dimsfiions,error,memspace, filespace(i))
                    else
                       ...
                       call h5dwrite_real_1(dset_id(i), H5T_NATIVE_REAL, &
                            temporary0_vec(1:all_strd_lecs_lng(procn+1)),  &
                            dimsfilecs,error,memspace, filespace(i))
...
#ifdef MPI
                 if(i .le. midvars) then
                    if (all_strd_ions_lng(procn+1) .ne. 0) then
                       if (varname .eq. 'indi' .or. varname .eq. &
                            'proci') then
                          call MPI_Send(int_temp_vec, &
                               all_strd_ions_lng(procn+1), &
                               mpi_integer,0,1,MPI_COMM_WORLD,error)
                       else
                          call MPI_Send(temporary_vec, &
                               all_strd_ions_lng(procn+1), &
                               mpi_real,0,1,MPI_COMM_WORLD,error)
                       endif
                    endif
                 else
                    if (all_strd_lecs_lng(procn+1) .ne. 0) then
...
!  integer*8 all_ions_lng(size0), all_lecs_lng(size0)
  ...
  if(lap .ge.pltstart .and. modulo((lap-pltstart),torqint) .eq.0)then
     ...
     fnamefld=trim(output_dir_name)//"/flds.hug."//trim(indchar)
