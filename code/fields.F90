...
!-------------------------------------------------------------------------------
	...
	real(dprec) :: xinject, xinject2, xinject3, yinject, yinject2, x1in, x2in,y1in, y2in

	real(dprec) :: z1in, z2in !xinject -- where particles launched from, xin1,2 -- where particles should be removed from
...
!-------------------------------------------------------------------------------
	...
	public :: radiationx, radiationy, radiationz, periodicx, periodicy, periodicz, &
			  ...
			  highorder, corr, x1in, x2in, y1in, y2in, z1in, z2in, &
                          nghost, nghostz
       public :: xglob, xglob8, yglob, zglob, iglob, jglob, kglob, iloc, jloc, kloc
...
!	mzlast=mz0-((mz0-5)/sizez)*(sizez-1)
	...
	if (irestart .ne. 1) then
		mx=(mx0-nghost)/sizex+nghost	
		my=(my0-nghost)/sizey+nghost
		mz=(mz0-nghostz)/sizez+nghostz

		if(modulo(rank,sizex) .eq. sizex-1 ) then	
			if(mx0 .ne. (mx-nghost)*sizex+nghost) mx=mx0-(mx-nghost)*(sizex-1)
		endif
		if(modulo(rank,sizex*sizey)/sizex .eq. sizey-1) then	
			if(my0 .ne. (my-nghost)*sizey+nghost) my=my0-(my-nghost)*(sizey-1)
		endif	
		...
		if(rank/(sizex*sizey) .eq. sizez-1) then	
			if(mz0 .ne. (mz-nghostz)*sizez+nghostz) mz=mz0-(mz-nghostz)*(sizez-1)
		endif
	...
	if (irestart .eq. 1) then
		...
		i1=1
		i2=sizex
		mx0=sum(mxl(i1:i2)-nghost)+nghost
		j1=1
		j2=sizex*(sizey-1)+1
		my0=sum(myl(j1:j2:sizex)-nghost)+nghost
		...
		mz0=sum(mzl(k1:k2:(sizex*sizey))-nghostz)+nghostz
#endif	
	...
	mxcum=sum(mxl(i1:i2)-nghost)-(mxl(i2)-nghost)
		
	j1=1
	j2=modulo(rank,sizex*sizey)/sizex*sizex+1
	mycum=sum(myl(j1:j2:sizex)-nghost)-(myl(j2)-nghost)
	
#ifndef twoD	
	k1=1
	k2=rank/(sizex*sizey)*(sizex*sizey)+1
	mzcum=sum(mzl(k1:k2:(sizex*sizey))-nghostz)-(mzl(k2)-nghostz)
#endif	
...
real(dprec) function xglob8(xlocal)
implicit none
real :: xlocal
  xglob8 = xlocal + mxcum
end function xglob8 
...
!    do j=1,my
!       do i=1,mx
!          cury(i,j,k)=exp(-(xglob(i*1.)-lap*c)**2/(2*5**2))*1e-4*lap
!       enddo
!    enddo
! enddo
